...
# Import available configuration
...
# from ..processing import SensoryBuffer

# Use basic logging for now until utils.logging is available
...
class CognitiveAgent:
    ...
    async def _process_sensory_input(self, input_data: str, input_type: str) -> Dict[str, Any]:
        ...
        try:
            # Search across both STM and LTM for relevant context
            search_query = processed_input.get("raw_input", "")
            relevant_memories = self.memory.search_memories(search_query, max_results=5)
            
            # Convert memory objects to dictionaries for processing
            context_memories = []
            for memory, relevance, source in relevant_memories:
    ...
    async def _consolidate_memory(
        ...
        try:
            # Create memory content that includes both input and response
            memory_content = f"User: {input_data}\nAssistant: {response}"
            
            # Calculate importance based on attention scores
            importance = attention_scores["overall_attention"]
            
            # Store in short-term memory first
            self.memory.stm.store(
                content=memory_content,
                importance=importance
            )
            
            # Update conversation context for immediate access
            memory_entry = {
                "input": input_data,
                "response": response,
            ...
            # Fallback to conversation context only
            memory_entry = {
                "input": input_data,
                "response": response,
                "timestamp": datetime.now(),
    ...
    def get_cognitive_status(self) -> Dict[str, Any]:
        ...
        return {
            "session_id": self.session_id,            "fatigue_level": self.current_fatigue,
    ...
    async def enter_dream_state(self):
        ...
        try:
            # Trigger memory consolidation and dream processing
            dream_results = self.memory.enter_dream_state(duration_minutes=5)
            
            # Reset fatigue after dream processing
            self.current_fatigue = 0.0
            
            logger.info(f"Dream state processing completed: {dream_results}")
            
        except Exception as e:
            logger.error(f"Dream state processing failed: {e}")
            # Still reset fatigue even if consolidation fails
            self.current_fatigue = 0.0
