                    ...
                    if memory_id not in self.memories:
                        ...
                        try:
                            record = LTMRecord(
                                ...
                                encoding_time=datetime.fromisoformat(metadata.get('encoding_time', datetime.now().isoformat())),
                                last_access=datetime.fromisoformat(metadata.get('last_access', datetime.now().isoformat())),
                                ...
                                consolidation_count=metadata.get('consolidation_count', 0)
                            )
        ...
        try:
            ...
            if results['ids'] and results['ids'][0]:
                for i, memory_id in enumerate(results['ids'][0]):
                    # Get the record from in-memory store
                    record = self.memories.get(memory_id)
                    if record:
        ...
        if memory_id in self.memories:
            ...
            for tag in record.tags:
                if tag in self.tags_index and memory_id in self.tags_index[tag]:
                    ...
                    if not self.tags_index[tag]:  # Remove empty tag lists
